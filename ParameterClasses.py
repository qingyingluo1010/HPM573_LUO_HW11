...
def add_background_mortality(prob_matrix):
    ...
    rate_matrix = MarkovCls.discrete_to_continuous(prob_matrix, 1) #delta t is 1 year
    # add mortality rates
    for s in HealthStats:
...
def annual_rate_of_first_ever_stroke(prob_matrix):
    ...
    for s in HealthStats:
        if s not in [HealthStats.HIV_DEATH, HealthStats.BACKGROUND_DEATH]:
            rate_matrix[s.value][HealthStats.BACKGROUND_DEATH.value] \
                = -np.log(1 - Data.annual_rate_of_first_ever_stroke)
